...
const MAX_PLACE_PHOTOS = 15
const SUPABASE_DATA_ROW_ID = 'main'
...
const normalizePlacePhotos = (place) => {
  const photos = Array.isArray(place?.photos)
    ? place.photos.filter(Boolean).slice(0, MAX_PLACE_PHOTOS)
    : []
  if (photos.length > 0) return photos
  return place?.photo ? [place.photo] : []
}

const getPlaceCoverPhoto = (place) => normalizePlacePhotos(place)[0] || null
...
function normalizeLoadedData(data) {
  ...
  if (!normalized.stories && Array.isArray(normalized.itineraries)) {
    normalized.stories = normalized.itineraries.map(migrateItineraryToStory)
    delete normalized.itineraries
  }
  if (!Array.isArray(normalized.stories)) normalized.stories = INIT_STORIES
  if (!Array.isArray(normalized.places)) normalized.places = INIT_PLACES
  normalized.places = normalized.places.map((place) => ({
    ...place,
    photos: normalizePlacePhotos(place)
  }))
  if (!Array.isArray(normalized.comments)) normalized.comments = []
  if (!Array.isArray(normalized.feedback)) normalized.feedback = []
  return normalized
}
...
async function loadData() {
  try {
    ...
    return normalizeLoadedData(data.payload)
  } catch (err) {
...
function parseStoredBackup(raw) {
  if (raw == null || raw === '') return null
  let parsed = typeof raw === 'string' ? JSON.parse(raw) : raw
  // Some exports/localStorage copies are double-encoded JSON strings.
...
function PlaceCard({ place, onClick }) {
  ...
  return (
    <div className="card place-card" onClick={onClick}>
      {coverPhoto && <img src={coverPhoto} alt={place.name} className="place-card-photo" />}
...
function PlaceDetail({ place, comments, onBack, onAddComment }) {
  const meta = CATEGORY_META[place.category] || {}
  const coverPhoto = getPlaceCoverPhoto(place)
  const photos = normalizePlacePhotos(place)
  ...
  return (
    <div>
      ...
      <div className="hero">
        {coverPhoto && <img src={coverPhoto} alt={place.name} />}
        ...
        <div className="section">
          ...
          <div className="gallery-grid">
            {photos.map((photo, idx) => (
              <div key={`${place.id}-photo-${idx}`} className="gallery-item">
                <img src={photo} alt={`${place.name} ${idx + 1}`} />
              </div>
            ))}
          </div>
        </div>
        ...
        <section className="chapter-section">
          ...
          <div className="gallery-grid">
            {story.gallery.map((g, i) => {
              ...
              const photo = g.photo || getPlaceCoverPhoto(linkedPlace)
...
function PlaceEditor({ place, onSave, onCancel }) {
  ...
  const handlePhotos = async (e) => {
    ...
    const remainingSlots = MAX_PLACE_PHOTOS - form.photos.length
    if (remainingSlots <= 0) {
      alert(`You can upload up to ${MAX_PLACE_PHOTOS} photos per place.`)
      return
    }
    ...
    try {
      ...
      for (const file of selectedFiles) {
        ...
        const path = `${form.id}/${uid()}-${Date.now()}.${ext}`
      ...
      setForm((f) => ({ ...f, photos: [...f.photos, ...uploadedUrls].slice(0, MAX_PLACE_PHOTOS) }))
      if (files.length > selectedFiles.length) {
        alert(`Only ${remainingSlots} more photo${remainingSlots > 1 ? 's' : ''} can be added.`)
      }
    } catch (err) {
      console.error('Photo upload failed:', err)
      alert('Image upload failed. Check Supabase bucket/policies and try again.')
    }
  }
  ...
  const submit = (e) => {
    ...
    onSave({
      ...form,
      photos: form.photos.slice(0, MAX_PLACE_PHOTOS),
  ...
  return (
    <form onSubmit={submit} className="card" style={{ padding: 22 }}>
      ...
      <div className="form-group">
        <label className="form-label">Photos (optional, up to {MAX_PLACE_PHOTOS})</label>
        <input type="file" accept="image/*" multiple onChange={handlePhotos} style={{ fontSize: 13 }} />
        <div className="muted" style={{ marginTop: 6 }}>
          First photo is used as the homepage cover image automatically.
        </div>
        {form.photos.length > 0 && (
          <div style={{ marginTop: 10 }}>
            <div className="gallery-grid">
              {form.photos.map((photo, idx) => (
                <div key={`${form.id}-photo-${idx}`} className="gallery-item" style={{ aspectRatio: 'unset', height: 120 }}>
                  <img src={photo} alt="" />
                  <div className="gallery-item-caption">
                    <span>{idx === 0 ? 'Cover' : `Photo ${idx + 1}`}</span>
                    <button
                      type="button"
                      className="btn btn-ghost"
                      style={{ color: '#FAF7F2', padding: 0, minHeight: 'auto' }}
        ...
        {form.gallery.length > 0 && (
          <div className="editor-section-body">
            {form.gallery.map((g, i) => {
              const linkedPlace = g.placeId ? places.find(p => p.id === g.placeId) : null
              const previewSrc = g.photo || getPlaceCoverPhoto(linkedPlace)
              return (
...
function AdminTab({ places, stories, comments, feedback, onSavePlace, onDeletePlace, onSaveStory, onDeleteStory, onDeleteComment, onDeleteFeedback, onExport, onImportData }) {
      ...
      {section === 'places' && (
        <>
          <div className="card" style={{ padding: 16, marginBottom: 16 }}>
            <h3 style={{ fontSize: 16, marginBottom: 8 }}>Sync to Supabase</h3>
            <p className="muted" style={{ marginBottom: 12 }}>
              <strong>localhost</strong> and <strong>wanderlaine.vercel.app</strong> use separate browser storage.
              If you see 594 on Vercel but not here, export from Vercel first (steps below), then import the JSON file here.
            </p>
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 12 }}>
              <button type="button" className="btn btn-primary" onClick={onImportData.fromBrowser}>
                Import from this browser → Supabase
              </button>
              ...
              <input
                ...
                style={{ display: 'none' }}
                onChange={onImportData.fromFile}
              />
            </div>
            <details className="muted" style={{ marginBottom: 12, fontSize: 13 }}>
              <summary style={{ cursor: 'pointer', fontWeight: 600, color: '#2C2825' }}>Export 594 places from Vercel (one-time)</summary>
              <ol style={{ margin: '10px 0 0 18px', lineHeight: 1.6 }}>
                <li>Open <a href="https://wanderlaine.vercel.app/" target="_blank" rel="noreferrer">wanderlaine.vercel.app</a></li>
                <li>Press F12 → Console, paste and run the download script shown in our chat</li>
                <li>Come back to localhost Admin → <strong>Import JSON file</strong></li>
              </ol>
            </details>
            <label className="form-label">Or paste JSON backup</label>
            <textarea
              className="textarea"
              rows={4}
              value={pasteJson}
              onChange={(e) => setPasteJson(e.target.value)}
              placeholder='Paste full {"places":[...],"stories":[...]} JSON'
            ...
            <button
              ...
              onClick={() => onImportData.fromPaste(pasteJson)}
            >
              Import pasted JSON → Supabase
            </button>
          </div>
          <div className="admin-bar">
            <div>
              <h3 style={{ fontSize: 18, marginBottom: 2 }}>Manage places</h3>
              <div className="muted">{places.length} total · Supabase persistence</div>
            </div>
    ...
    <>
      <div className="admin-bar">
        <div>
          ...
          <div className="muted">
            {comments.length} place comment{comments.length !== 1 ? 's' : ''} · {feedback.length} general feedback · Supabase persistence
          </div>
        </div>
      </div>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    let cancelled = false
    ;(async () => {
      const result = await saveData({ places, stories, comments, feedback })
      if (cancelled) return
      if (!result.ok) {
        console.error('Failed to save data to Supabase:', result.error)
        if (!saveFailed) {
          setToast('Save failed: cannot write to Supabase. Please try again.')
          setSaveFailed(true)
        }
        return
      }
    ...
    return () => { cancelled = true }
  }, [places, stories, comments, feedback, saveFailed, isHydrated])
  ...
  const handleExport = () => {
    const blob = new Blob([JSON.stringify({ places, stories, comments, feedback }, null, 2)], { type: 'application/json' })
  ...
  const applyImportedData = (normalized) => {
    if (!normalized?.places?.length) {
      setToast('Import found 0 places — check your backup file.')
      return
    }
    ...
    setToast(`Imported ${normalized.places.length} places — saving to Supabase…`)
  }

  const handleImportFromBrowser = () => {
    try {
      const raw = localStorage.getItem('wanderlaine-data')
      if (!raw) {
        setToast('No data on this site. Export JSON from wanderlaine.vercel.app, then use Import JSON file here.')
        return
      }
      applyImportedData(parseStoredBackup(raw))
    } catch (err) {
      console.error('Import from browser failed:', err)
      setToast('Invalid JSON in localStorage. Try Import JSON file instead.')
    }
  }

  const handleImportFromPaste = (raw) => {
    try {
      applyImportedData(parseStoredBackup(raw))
    } catch (err) {
      console.error('Import from paste failed:', err)
      setToast('Invalid pasted JSON.')
    }
  }
  ...
  const handleImportFromFile = (e) => {
    ...
    reader.onload = () => {
      try {
        applyImportedData(parseStoredBackup(reader.result))
      } catch (err) {
      ...
      <div className="app" style={{ paddingTop: 20 }}>
        ...
        {activeTab === 'admin' && (
          <AdminTab
            ...
            onImportData={{
              fromBrowser: handleImportFromBrowser,
              fromFile: handleImportFromFile,
              fromPaste: handleImportFromPaste
            }}
          />
        )}
      </div>
